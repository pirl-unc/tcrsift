...
class TestLoadSct:
    ...
    def test_load_sct_basic(self, sct_excel_file):
        ...
        df = load_sct(
            sct_excel_file, min_snr=0, min_reads_per_chain=0, require_mutation_match=False
        )
        assert len(df) == 3
    ...
    def test_load_sct_snr_filter(self, sct_excel_file):
        """Test SNR filtering."""
        df = load_sct(
            sct_excel_file, min_snr=2.5, min_reads_per_chain=0, require_mutation_match=False
        )
    ...
    def test_load_sct_read_count_filter(self, sct_excel_file):
        ...
        df = load_sct(
            sct_excel_file, min_snr=0, min_reads_per_chain=40, require_mutation_match=False
        )
