...
def calc_thresholds_and_counts(
...
) -> tuple[dict[float, float], dict[float, int], sm.Logit]:
    fdr_to_threshold = {}
    threshold_to_count = {}
    ...
    if weight < 0:
        print(
            f"-- Data too messy to choose thresholds adaptively, defaulting to {default_freq_threshold:0.2f}"
        )
        for fdr in fdrs:
            fdr_to_threshold[fdr] = default_freq_threshold
            threshold_to_count[default_freq_threshold] = count_at_threshold(
                df, default_freq_threshold
            )
    else:
